...
bearing과 align을 분리한 게 의도적이다. bearing만 있으면 "목표점이 어느 쪽인가"는 알지만 **도착했을 때 어느 방향을 보고 있어야 하는가**를 모른다. align이 그 자세 조건을 담는다. yaw rate는 복구 상황에서 특히 중요하다 — 차체가 이미 돌기 시작했는지를 알려주는 신호이기 때문이다.
